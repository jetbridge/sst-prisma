...
export const handler = async (): Promise<string> => {
  ...
  if (listMigrationDirectoriesResult.migrations.length > 0) {
    const { migrations } = listMigrationDirectoriesResult;
    console.info(`${migrations.length} migration${migrations.length > 1 ? 's' : ''} found in migrations`);
  } else {
    throw new Error('No migrations found in migrations');
  }

  const { editedMigrationNames } = diagnoseResult;
